...
class AdminRequestDetailPage extends StatelessWidget {
          ...
          child: Padding(
            ...
            child: Column(
              children: [
                Row(
                  children: [
                    Container(
                      ...
                      child: const Icon(Icons.insert_drive_file, size: 36, color: Colors.teal),
                    ),
                    ...
                    Container(
                      ...
                      decoration: BoxDecoration(
                        color: request.status == "Approved" ? Colors.green[50] : Colors.orange[50],
                        borderRadius: BorderRadius.circular(8),
                      ),
                      child: Text(
                        request.status,
                ...
                _keyValueRow("Date / Time", request.dateTime),
                const Divider(),
                _keyValueRow("Asset ID", request.assetId),
                const Divider(),
                _keyValueRow("Asset Type", "Cable"), // replace with real field if available
                const Divider(),
                _keyValueRow("User Name", request.requesterName),
                const SizedBox(height: 16),
                Align(
                  alignment: Alignment.centerLeft,
                  child: Text("Reason", style: TextStyle(fontWeight: FontWeight.w600, color: Colors.grey[700])),
                ),
  ...
  Widget _keyValueRow(String key, String value) {
    return Padding(
      ...
      child: Row(
        children: [
          Expanded(flex: 3, child: Text("$key", style: const TextStyle(color: Colors.black54))),
